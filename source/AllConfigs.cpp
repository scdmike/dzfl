...
QString AllConfigs::checkingValue(){
    ...
    //排队地址验证
    if(!QtPing(SettingKeyValue["评价服务器IP"])){
        if(errorKey!="")errorKey=errorKey+",";
        errorKey = errorKey+"评价服务器IP,评价服务器Port";
    }
    //电视机ip地址验证
    return errorKey;
}
...
//        QString Queueip = CsGetConfig("评价服务器IP");
//        QString Queueport = CsGetConfig("评价服务器Port");
//        QString urlForSp = "http://"+Queueip+":"+Queueport+"/ServiceEntry.aspx";
//        QString host;
//        int port ;
//        QString url;
//        Record cmdBiz;
//        cmdBiz["Service"]= "VCall.GetSetting";
